...
percent_to_borrow = 0.75
testnet_get_weth = False


def main():
    account = get_account()
    erc20_address = config["networks"][network.show_active()]["weth_token"]
    if network.show_active() in ["mainnet-fork"] or testnet_get_weth:
        get_weth(account)
    ...
    amount_borrowing = borrowable_eth * percent_to_borrow
    erc20_eth_price = get_asset_price()
    amount_erc20_to_borrow = (amount_borrowing / erc20_eth_price) * 0.98
    print(
        f"We are going to borrow {percent_to_borrow*100}% of our borrowable ETH,\n"
        + "which is {amount_erc20_to_borrow} DAI"
    )
